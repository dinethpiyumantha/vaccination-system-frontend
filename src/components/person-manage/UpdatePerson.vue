...
<div>
  <h4 class="mb-4">Update Person</h4>
  ...
  <a-form-model ref="ruleForm" :model="form" :label-col="labelCol" :wrapper-col="wrapperCol" :rules="rules">
    ...
    <a-form-model-item label="National ID (NIC)" has-feedback ref="nic" prop="nic">
      <a-input v-model="form.nic" :change="birthDayExtractorI()" :disabled="true">
    ...
    <a-form-model-item label="Age"  ref="age" prop="age">
      <a-input v-model="form.age" :disabled="true"/>
    </a-form-model-item>

    <!-- Gender Selection Input -->
    <a-form-model-item label="Gender" ref="gender" prop="gender">
      <a-select v-model="form.gender" placeholder="Please select gender" :change="serialNoGenerate()" :disabled="true">
...
<script>
// Import local data files and libraries
...
export default {
  ...
  created() {
    this.$http.get('http://127.0.0.1:8000/api/person/get/'+this.$route.params.id).then(function (response) {
      this.form = response.data.person;
