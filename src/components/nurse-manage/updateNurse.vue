      ...
      <div class="row">
        <div class="col-6">
          ...
          <a-form-model-item has-feedback label="Joined Date" prop="joined_date" >
            <a-date-picker v-model="nurseForm.joined_date" style="width: 100%" :disabled="disabled"></a-date-picker>
          </a-form-model-item>
        ...
        <div class="col-6 pl-0">
          ...
          <a-form-model-item label="From" prop="From" has-feedback :label-col="rightLabelCol" :wrapperCol="rightWrapCol">
            <a-date-picker v-model="nurseForm.From" style="width: 100%" />
          </a-form-model-item>

          <a-form-model-item label="To" prop="To" has-feedback :label-col="rightLabelCol" :wrapperCol="rightWrapCol">
...
export default {
  data() {
    //a function to validate the age
    let checkPending; //a variable 
  ...
  methods: {
    submitForm() {
      this.$confirm({
        title: 'Do you really want to update nurse details? ',
        content: '',
        okText: 'Yes',
        cancelText: 'No', 
        onOk:() => {
          this.$refs.ruleForm.validate(valid => {
            if (valid) {
              ...
              this.$http.put('http://127.0.0.1:8001/api/nurses/update/'+this.$route.params.id, this.nurseForm).then(function (response){
                this.openNotificationSuccess("Nurse updated successfully!", "Entry updated");
  ...
  created(){
      this.$http.get('http://127.0.0.1:8001/api/nurses/get/'+this.$route.params.id).then(function(response){
          this.nurseForm = response.data.nurse;
